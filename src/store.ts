...
export const useStore = create<AppState>((set, get) => {
  ...
  const initialModel = localStorage.getItem(`faust_model_${initialProvider}`) || (initialProvider === 'moonshot' ? 'kimi-k2.5' : '');
  const initialKeys: Record<AIProvider, string> = {
    moonshot: localStorage.getItem('faust_api_key_moonshot') || '',
    gemini: localStorage.getItem('faust_api_key_gemini') || '',
    deepseek: localStorage.getItem('faust_api_key_deepseek') || '',
    glm: localStorage.getItem('faust_api_key_glm') || '',
  };
    ...
    setSettings: async (settings) => {
      ...
      if (settings.provider && settings.provider !== current.provider) {
        ...
        if (!newModel && newApiKey) {
          switch(newProvider) {
            case 'moonshot': newModel = 'kimi-k2.5'; break;
            case 'gemini': newModel = 'gemini-2.0-flash-thinking-exp-01-21'; break;
            case 'deepseek': newModel = 'deepseek-chat'; break;
            case 'glm': newModel = 'glm-4-plus'; break;
          }
        }
      }
