...
// Safe localStorage wrapper to handle "Tracking prevention blocked access to storage" errors
const safeLocalStorage = {
  getItem: (key: string): string | null => {
    try {
      return localStorage.getItem(key);
    } catch (e) {
      console.warn(`[DSPCLAW] Storage access blocked for key: ${key}`, e);
      return null;
    }
  },
  setItem: (key: string, value: string): void => {
    try {
      ...
      console.warn(`[DSPCLAW] Storage write blocked for key: ${key}`, e);
    }
  },
  removeItem: (key: string): void => {
    try {
      ...
      console.warn(`[DSPCLAW] Storage remove blocked for key: ${key}`, e);
    }
  }
};
...
export const useStore = create<AppState>((set, get) => {
  ...
  const initialModel = safeLocalStorage.getItem(`faust_model_${initialProvider}`) || (initialProvider === 'moonshot' ? 'kimi-k2.5' : '');
  ...
  return {
    ...
    setSettings: async (settings) => {
      ...
      const newCustomBaseUrl = settings.customBaseUrl !== undefined ? settings.customBaseUrl : current.customBaseUrl;
      let newModels = settings.models || current.models;

      if (newCustomBaseUrl !== current.customBaseUrl) {
        safeLocalStorage.setItem('faust_custom_base_url', newCustomBaseUrl);
      }

      if (settings.provider && settings.provider !== current.provider) {
        newApiKey = newApiKeys[newProvider];
        newModels = []; // CRITICAL: Reset model list
        
        // Attempt to load a saved model for the new provider, or set a default
        const savedModel = safeLocalStorage.getItem(`faust_model_${newProvider}`);
        if (savedModel) {
          newModel = savedModel;
        } else if (newApiKey) { // Only set a default if there's a key
          switch(newProvider) {
